...
<body>
	...
	<%	
		...
		if(cp == null || cp.trim().equals("")){
			//default 값 설정
			cp = "1"; // 5개씩 묶은 것 중에서 1번째 페이지 보겠다.(1~5번페이지 보여줌) 
		}
		...
		if(totalboardcount % pagesize == 0){
			pagecount = totalboardcount / pagesize; //5개씩 딱 나눠떨어질때 
		}else{ 
			pagecount = (totalboardcount / pagesize) + 1; //나머지가 있을때 한페이지 더
		}
			...
			<table width="80%" border="1" cellspacing="0" align="center">
				...
				<c:forEach var="board" items="<%=list%>">
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left">
							...
							<a href="<%=request.getContextPath()%>/content.do?idx=${board.idx}&cp=${cpage}&ps=${pagesize}">
								<c:choose>
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="<%=request.getContextPath()%>/list.do?cp=${cpage-1}&ps=${pagesize}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="<%=request.getContextPath()%>/list.do?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="<%=request.getContextPath()%>/list.do?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
				...
				<tr>
					...
					<%
						...
						ThePager  pager = new ThePager(totalboardcount,cpage,pagesize,pagersize,"list.do");
					%>
					<%= pager.toString() %>
					</td>
			</table>
