				...
				<table width="80%" border="1">
					...
					<tr>
						<td colspan="4" align="center"><a
							href="<%=request.getContextPath()%>/list.do?cp=<%=cpage%>&ps=<%=pagesize%>">목록가기</a> |<a
							href="<%=request.getContextPath()%>/edit.do?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>">편집</a>
							|<a
							href="<%=request.getContextPath()%>/delete.do?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>">삭제</a>
							|<a
							href="<%=request.getContextPath()%>/rewrite.do?idx=<%=idx%>&cp=<%=cpage%>&ps=<%=pagesize%>&subject=<%=board.getSubject()%>">답글</a>
						</td>
					</tr>
				</table>
				<!--  꼬리글 달기 테이블 -->
